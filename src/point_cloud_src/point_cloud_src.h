...
{
  public:
    BackProjectorFocalLengthPrincipalPoint(const float2 fl,
                                           const float2 pp,
                                           const float2 range)
        : _fl(fl), _pp(pp), _range(range)
    {
    }
    void backProjectDepthMap(const float *depthMap,
                             float4 *vertMap,
                             const int width,
    ...
    void backProjectDepthMap(const ushort *depthMap,
                             float4 *vertMap,
                             const int width,
                             const int height) const
    {
...
{
  public:
    BackProjectorFocalLengthPrincipalPointScaled(const float2 fl,
                                                 const float2 pp,
                                                 const float2 range,
                                                 const float scale)
    ...
    {
        depthToVertices(
            depthMap, vertMap, width, height, _pp, _fl, _range, _scale);
    }
    void backProjectDepthMap(const ushort *depthMap,
                             float4 *vertMap,
                             const int width,
...
{
  public:
    BackProjectorCalibrationParams(const float *calibrationParams,
                                   const float2 range)
        : _range(range), _calibrationParams(9)
    {
        memcpy(
            _calibrationParams.hostPtr(), calibrationParams, 9 * sizeof(float));
    ...
    void backProjectDepthMap(const float *depthMap,
                             ...
                             const int height) const
    {
        depthToVertices(depthMap,
                        vertMap,
                        width,
                        height,
                        _calibrationParams.devicePtr(),
                        _range);
    }
    void backProjectDepthMap(const ushort *depthMap,
                             float4 *vertMap,
...
{
  public:
    BackProjectorCalibrationParamsScaled(const float *calibrationParams,
                                         const float2 range,
                                         const float scale)
...
class PointCloudSourceBase
{
  public:
    ...
    inline ColorLayout getColorLayout() const
    {
    ...
    virtual uint getDepthHeight() const
    {
        return _depthSource->getDepthHeight();
    }
    inline uint getColorWidth() const { return _depthSource->getColorWidth(); }
    inline uint getColorHeight() const
    {
        return _depthSource->getColorHeight();
    }
    inline uint getFrame() const { return _depthSource->getFrame(); }
    inline void setFrame(const uint frame) { _depthSource->setFrame(frame); }
    virtual float2 getFocalLength() const
    {
        return _depthSource->getFocalLength();
    }
    inline float2 getPrincipalPoint() const
    {
        return _depthSource->getPrincipalPoint();
    }
    ...
    {
        memcpy(
            _vertMap->hostPtr(), vertMap, _vertMap->length() * sizeof(float4));
    ...
    {
        dart::cropBox(
            _vertMap->devicePtr(), getDepthWidth(), getDepthHeight(), min, max);
        _vertsOnHost = false;
    }

    inline void maskPointCloud(const int *deviceMask)
    {
        dart::maskPointCloud(_vertMap->devicePtr(),
                             getDepthWidth(),
                             getDepthHeight(),
                             deviceMask);
        _vertsOnHost = false;
    ...
    inline void setFocalLength(const float2 focalLength)
    {
        ...
        if (s == 1.0f)
        {
            _backProjector = new BackProjectorFocalLengthPrincipalPoint(
                _depthSource->getFocalLength(),
                _depthSource->getPrincipalPoint(),
                make_float2(0, 3));
        }
        else
        {
            _backProjector = new BackProjectorFocalLengthPrincipalPointScaled(
                _depthSource->getFocalLength(),
                _depthSource->getPrincipalPoint(),
                make_float2(0, 3),
                s);  // TODO
  ...
  public:
                     ...
                     float2 range)
    {
        ...
        if (depthSource->hasRadialDistortionParams())
        {
            ...
            memcpy(&calibrationParams[4],
                   depthSource->getRadialDistortionParams(),
                   5 * sizeof(float));
            if (s != 1.0f)
            {
                _backProjector = new BackProjectorCalibrationParamsScaled(
                    calibrationParams, range, s);
            }
            else
            {
                _backProjector = new BackProjectorCalibrationParams(
                    calibrationParams, range);
            }
        }
        else
        {
            ...
            {
                _backProjector =
                    new BackProjectorFocalLengthPrincipalPointScaled(
                        depthSource->getFocalLength(),
        ...
        uint w = depthSource->getDepthWidth();
        uint h = depthSource->getDepthHeight();
        ...
        cudaMalloc(&_dFilteredDepth,
                   depthSource->getDepthWidth() *
                       depthSource->getDepthHeight() * sizeof(float));
        CheckCudaDieOnError();
    ...
    {
        DepthSource<DepthType, ColorType> *depthSource =
            static_cast<DepthSource<DepthType, ColorType> *>(_depthSource);
        if (_filteredNorms)
        {
            bilateralFilter(depthSource->getDeviceDepth(),
                            _dFilteredDepth,
                            depthSource->getDepthWidth(),
            ...
            verticesToNormals(_vertMap->devicePtr(),
                              _normMap->devicePtr(),
                              depthSource->getDepthWidth(),
                              depthSource->getDepthHeight());
            if (!_filteredVerts)
            {
                _backProjector->backProjectDepthMap(
                    depthSource->getDeviceDepth(),
                    _vertMap->devicePtr(),
                    depthSource->getDepthWidth(),
                    depthSource->getDepthHeight());
            }
