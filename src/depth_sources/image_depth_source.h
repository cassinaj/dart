...
class ImageDepthSource : public DepthSource<DepthType, ColorType>
{
  public:
    ...
    /**
    * An initializer which uses printf formatting to locate depth and color
    * frames.
    ...
    */
    ...
    /**
    * An initializer which locates depth and color frames by iterating over
  ...
  private:
    ...
    bool readPNG(const char* filename, bool isDepth = true);
    bool getDimPNG(const char* filename, uint& width, uint& height);
    bool readJPG(const char* filename, bool isDepth = false);
    bool getDimJPG(const char* filename, uint& width, uint& height);
...
template <typename DepthType, typename ColorType>
bool
ImageDepthSource<DepthType, ColorType>::initialize(
    const std::string& depthDirectory,
    const ImageType depthImageType,
    const float2 focalLength,
    ...
    const std::vector<long>* colorTimes)
{
    ...
    {
        std::cerr << "could not open directory " << _depthDirectory
                  << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "no files with extension " << expectedExtension
                  << " in directory " << _depthDirectory << std::endl;
        return false;
    }

    std::cout << "found " << _depthFilenames.size() << " depth frames"
              << std::endl;
    ...
    else
    {
        ...
        switch (_depthImageType)
        {
            case IMAGE_PNG:
                getDimPNG(
                    firstFile.c_str(), this->_depthWidth, this->_depthHeight);
                break;
            case IMAGE_JPG:
                getDimJPG(
                    firstFile.c_str(), this->_depthWidth, this->_depthHeight);
    ...
    {
        this->_principalPoint =
            make_float2(this->_depthWidth / 2, this->_depthHeight / 2);
    }
    else
    {
...
// allocate data
#ifdef CUDA_BUILD
    _depthData =
        new MirroredVector<DepthType>(this->_depthWidth * this->_depthHeight);
#else
    ...
    if (hasColor)
    {
        ...
        {
            std::cerr << "could not open directory " << _colorDirectory
                      << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "no files with extension " << expectedExtension
                      << " in directory " << _colorDirectory << std::endl;
            return false;
        }

        std::cout << "found " << _colorFilenames.size() << " color frames"
                  << std::endl;
        ...
        else
        {
            ...
            {
                case IMAGE_PNG:
                    getDimPNG(firstFile.c_str(),
                              this->_colorWidth,
                              this->_colorHeight);
                    break;
                case IMAGE_JPG:
                    getDimJPG(firstFile.c_str(),
                              this->_colorWidth,
                              this->_colorHeight);
                    break;
            }
        ...
        if (colorTimes && depthTimes)
        {
            ...
            for (int i = 0; i < _depthTimes.size(); ++i)
            {
                ...
                colorIt =
                    std::lower_bound(colorIt, colorTimes->end(), targetTime);
                if (colorIt == colorTimes->end())
                {
                    --colorIt;
                    _correspondingColorFrames.push_back(colorIt -
                                                        colorTimes->begin());
                }
                else
                {
...
bool
ImageDepthSource<DepthType, ColorType>::initialize(
    const std::string depthFilenameFormat,
    const float2 focalLength,
    const float2 principalPoint,
    const uint depthWidth,
    const uint depthHeight,
...
{
    // set depth file type
    std::string depthExtension =
        depthFilenameFormat.substr(depthFilenameFormat.find_last_of(".") + 1);
    ...
    {
        std::cerr << "image extension " << depthExtension << " is not supported"
                  << std::endl;
        return false;
    }
    ...
    {
        std::string filename =
            stringFormat(depthFilenameFormat, _lastDepthFrame);
        struct stat buffer;
...
// allocate data
    ...
    {
        // set color file type
        std::string colorExtension = colorFilenameFormat.substr(
            colorFilenameFormat.find_last_of(".") + 1);
        ...
        {
            std::cerr << "image extension " << colorExtension
        ...
        {
            std::string filename =
                stringFormat(colorFilenameFormat, _lastColorFrame);
            struct stat buffer;
        ...
        {
            std::cerr << "no images with color filename format found"
                      << std::endl;
            return false;
        }

        std::string firstFile =
            stringFormat(colorFilenameFormat, firstColorFrame);
...
template <typename DepthType, typename ColorType>
void
ImageDepthSource<DepthType, ColorType>::setFrame(const uint frame)
{
    this->_frame = frame;
...
template <typename DepthType, typename ColorType>
void
ImageDepthSource<DepthType, ColorType>::advance()
{
    // update frame
    this->_frame++;
...
template <typename DepthType, typename ColorType>
void
ImageDepthSource<DepthType, ColorType>::readDepth()
{
    // read depth
    const std::string depthFilename =
        _depthDirectory + "/" + _depthFilenames[this->_frame];
    switch (_depthImageType)
    {
        case IMAGE_PNG:
    ...
    //    for (int i=0; i<this->_depthWidth*this->_depthHeight/(factor*factor);
    //    ++i) {
    //        noiseMap[i] = 2*nd(gen);
    //    }
    ...
    //            float tx = xx - px;
    //            float ty = yy - py;
    //            float noise = (1-tx)*(1-ty)*noiseMap[px +
    //            py*this->_depthWidth/factor] +
    //                    (1-tx)*ty*noiseMap[px +
    //                    (py+1)*this->_depthWidth/factor] +
    //                    tx*(1-ty)*noiseMap[(px+1) +
    //                    py*this->_depthWidth/factor] +
    //                    tx*ty*noiseMap[(px+1) +
    //                    (py+1)*this->_depthWidth/factor];
...
template <typename DepthType, typename ColorType>
void
ImageDepthSource<DepthType, ColorType>::readColor()
{
    //    std::cout << "looking for corresponding color frame to " <<
    //    this->_frame <<": " <<  _correspondingColorFrames[this->_frame];

    // read color
    int correspondingColorFrame = _correspondingColorFrames[this->_frame];
    const std::string colorFilename =
        _colorDirectory + "/" + _colorFilenames[correspondingColorFrame];
    switch (_colorImageType)
    {
        case IMAGE_PNG:
...
template <typename DepthType, typename ColorType>
bool
ImageDepthSource<DepthType, ColorType>::readPNG(const char* filename,
                                                const bool isDepth)
{
    ...
    png_structp pngPtr = png_create_read_struct(
        PNG_LIBPNG_VER_STRING, &buff, pngErrorHandler, NULL);
    if (!pngPtr)
    {
    ...
    //    std::cout << width << "x" << height << ", bit depth: " << bitDepth <<
    //    ", color type: " << colorType << std::endl;
    ...
    if (isDepth)
    {
        ...
        {
            std::cerr << "expected " << this->_depthWidth << "x"
                      << this->_depthHeight << " depth but got " << width << "x"
                      << height << std::endl;
            fclose(file);
            return false;
        }
        ...
        {
            std::cerr << "expected " << 8 * sizeof(DepthType)
                      << " bits but got " << bitDepth * channels << std::endl;
            fclose(file);
            return false;
        }
    }
    else
    {
        ...
        {
            std::cerr << "expected " << this->_colorWidth << "x"
                      << this->_colorHeight << " color but got " << width << "x"
                      << height << std::endl;
            fclose(file);
            return false;
        }
        ...
        {
            std::cerr << "expected " << 8 * sizeof(ColorType)
                      << " bits but got " << bitDepth * channels << std::endl;
            fclose(file);
            return false;
        }
    ...
    if (isDepth)
    {
        ...
        {
#ifdef CUDA_BUILD
            rowPointers[i] = ((png_bytep)_depthData->hostPtr()) +
                             i * png_get_rowbytes(pngPtr, infoPtr);
#else
            rowPointers[i] =
                ((png_bytep)_depthData) + i * png_get_rowbytes(pngPtr, infoPtr);
#endif  // CUDA_BUILD
        }
    }
    else
    {
        ...
        {
            rowPointers[i] =
                ((png_bytep)_colorData) + i * png_get_rowbytes(pngPtr, infoPtr);
        }
    }
...
template <typename DepthType, typename ColorType>
bool
ImageDepthSource<DepthType, ColorType>::getDimPNG(const char* filename,
                                                  uint& width,
                                                  uint& height)
{
    FILE* file = fopen(filename, "r");
...
template <typename DepthType, typename ColorType>
bool
ImageDepthSource<DepthType, ColorType>::readJPG(const char* filename,
                                                bool isDepth)
{
    ...
    if (isDepth)
    {
        ...
        {
            std::cerr << "expected " << sizeof(DepthType) << " bytes but got "
                      << outputComponents << std::endl;
            fclose(file);
            jpeg_finish_decompress(&cinfo);
            jpeg_destroy_decompress(&cinfo);
            return false;
...
#ifdef CUDA_BUILD
            rowPointers[v] =
                (unsigned char*)&this->_depthData->hostPtr()[v * width];
#else
    ...
    else
    {
        ...
        {
            std::cerr << "expected " << sizeof(ColorType) << " bytes but got "
                      << outputComponents << std::endl;
            fclose(file);
            jpeg_finish_decompress(&cinfo);
            jpeg_destroy_decompress(&cinfo);
    ...
    while (cinfo.output_scanline < height)
    {
        jpeg_read_scanlines(&cinfo,
                            rowPointers + cinfo.output_scanline,
...
template <typename DepthType, typename ColorType>
bool
ImageDepthSource<DepthType, ColorType>::getDimJPG(const char* filename,
                                                  uint& width,
                                                  uint& height)
{
    struct jpeg_decompress_struct cinfo;
