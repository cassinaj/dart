...
bool
OpenNIDepthSource::initialize(const char *deviceURI,
                              const bool getColor,
                              ...
                              const bool registerDepth)
{
    ...
    if (rc != openni::STATUS_OK)
    {
        std::cerr << "Could not create depth stream: "
                  << openni::OpenNI::getExtendedError() << std::endl;
        return false;
    }
    ...
    if (_isLive)
    {
        ...
        const openni::Array<openni::VideoMode> &modes =
            _device.getSensorInfo(openni::SENSOR_DEPTH)
        ...
        if (targetMode == -1)
        {
            std::cerr << "Could not find depth video mode " << depthWidth << "x"
                      << depthHeight << "@" << depthFPS << std::endl;
            return false;
        }
        ...
        if (rc != openni::STATUS_OK)
        {
            std::cerr << "Could not set depth video mode: "
                      << openni::OpenNI::getExtendedError() << std::endl;
            return false;
        }
        ...
        if (rc != openni::STATUS_OK)
        {
            std::cerr << "Could not set depth mirroring mode: "
                      << openni::OpenNI::getExtendedError() << std::endl;
            return false;
        }
    ...
    _focalLength =
        make_float2(525. * _depthWidth / 640, 525. * _depthWidth / 640);
    ...
    if (getColor)
    {
        ...
        if (rc != openni::STATUS_OK)
        {
            std::cerr << "Could not create color stream: "
                      << openni::OpenNI::getExtendedError() << std::endl;
            return false;
        }
        ...
        if (_isLive)
        {
            ...
            const openni::Array<openni::VideoMode> &modes =
                _device.getSensorInfo(openni::SENSOR_COLOR)
                    ->getSupportedVideoModes();
            for (int i = 0; i < modes.getSize(); ++i)
            {
            ...
            if (targetMode == -1)
            {
                std::cerr << "Could not find color video mode " << colorWidth
                          << "x" << colorHeight << "@" << colorFPS << std::endl;
                return false;
            }
            ...
            if (rc != openni::STATUS_OK)
            {
                std::cerr << "Could not set color video mode: "
                          << openni::OpenNI::getExtendedError() << std::endl;
                return false;
            }
            ...
            if (rc != openni::STATUS_OK)
            {
                std::cerr << "Could not set color mirroring mode: "
                          << openni::OpenNI::getExtendedError() << std::endl;
                return false;
            }
            ...
            if (rc != openni::STATUS_OK)
            {
                std::cerr << "Could not set frame sync: "
                          << openni::OpenNI::getExtendedError() << std::endl;
                return false;
            }

            rc = _device.setImageRegistrationMode(
                registerDepth ? openni::IMAGE_REGISTRATION_DEPTH_TO_COLOR
                              : openni::IMAGE_REGISTRATION_OFF);
            if (rc != openni::STATUS_OK)
            {
                std::cerr << "Could not set registration mode: "
                          << openni::OpenNI::getExtendedError() << std::endl;
                return false;
            }
    ...
    {
        std::cerr << "Could not start depth stream: "
                  << openni::OpenNI::getExtendedError() << std::endl;
        return false;
    }
...
const ushort *
OpenNIDepthSource::getDepth() const
{
    return (ushort *)_depthFrame.getData();
}

const ushort *
OpenNIDepthSource::getDeviceDepth() const
{
    return (ushort *)_deviceDepth;
}

const uchar3 *
OpenNIDepthSource::getColor() const
{
    return (uchar3 *)_colorFrame.getData();
}

uint64_t
OpenNIDepthSource::getDepthTime() const
{
    return _depthFrame.getTimestamp();
}

uint64_t
OpenNIDepthSource::getColorTime() const
{
    return _colorFrame.getTimestamp();
}

void
OpenNIDepthSource::setFrame(const uint frame)
{
    if (_isLive) return;
