...
{
  public:
    PredictionRenderer(const int width,
                       const int height,
    ...
    const float4 *getDevicePrediction() const
    {
        cudaDeviceSynchronize();
        return _dPrediction;
    }
    //    void renderPrediction(const Model & model,cudaStream_t & stream);
    //    void renderPrediction(const std::vector<const Model*> & models,
    //    cudaStream_t & stream);

    void raytracePrediction(const MirroredModel &model, cudaStream_t &stream);

    void raytracePrediction(const std::vector<const MirroredModel *> &models,
                            cudaStream_t &stream);

    void cullUnobservable(const float4 *dObsVertMap,
                          const int width,
                          const int height,
                          const cudaStream_t stream = 0);

    const unsigned char *getDebugBoxIntersection() const
    {
        return _debugBoxIntersections.hostPtr();
    }

    void debugPredictionRay(const std::vector<const MirroredModel *> &models,
                            const int x,
                            const int y,
