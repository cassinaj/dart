...
namespace dart
{
Optimizer::Optimizer(const DepthSourceBase *depthSource,
                     const int predictionWidth,
                     const int predictionHeight)
{
    const int pWidth =
        predictionWidth == -1 ? depthSource->getDepthWidth() : predictionWidth;
    const int pHeight = predictionHeight == -1 ? depthSource->getDepthHeight()
                                               : predictionHeight;

    const float2 focalLength = depthSource->getFocalLength() * pWidth /
                               (float)depthSource->getDepthWidth();

    init(depthSource->getDepthWidth(),
...
Optimizer::Optimizer(const int depthWidth,
...
void Optimizer::init(const int depthWidth,
                     const int depthHeight,
                     const float2 focalLength,
                     const int predictionWidth,
                     const int predictionHeight)
{
    std::cout << "predicting depth maps at " << predictionWidth << " x "
              << predictionHeight << std::endl;
    std::cout << "prediction focal length: " << focalLength.x << ", "
              << focalLength.y << std::endl;

    _predictionRenderer =
        new PredictionRenderer(predictionWidth, predictionHeight, focalLength);
    ...
    // scratch buffering
    cudaMalloc(&_dDebugObsToModNorm,
               predictionWidth * predictionHeight * sizeof(float4));
    cudaMalloc(&_dDebugModToObsNorm,
               predictionWidth * predictionHeight * sizeof(float4));
    cudaMalloc(&_dError, sizeof(float));

    cudaMalloc(&_dDebugDataAssocObsToMod,
               depthWidth * depthHeight * sizeof(int));
    cudaMemset(
        _dDebugDataAssocObsToMod, 0, depthWidth * depthHeight * sizeof(int));

    cudaMalloc(&_dDebugDataAssocModToObs,
               predictionWidth * predictionHeight * sizeof(int));
    cudaMemset(_dDebugDataAssocModToObs,
               0,
               predictionWidth * predictionHeight * sizeof(int));

    cudaMalloc(&_dDebugObsToModError, depthWidth * depthHeight * sizeof(float));
    cudaMemset(
        _dDebugObsToModError, 0, depthWidth * depthHeight * sizeof(float));

    cudaMalloc(&_dDebugModToObsError,
               predictionWidth * predictionHeight * sizeof(float));
    cudaMemset(_dDebugModToObsError,
               0,
               predictionWidth * predictionHeight * sizeof(float));

    cudaMalloc(&_dDebugIntersectionError,
               _maxIntersectionSites * sizeof(float));
    {
        std::vector<float> hostDebugIntersectionError(_maxIntersectionSites,
                                                      42);  // TODO
        cudaMemcpy(_dDebugIntersectionError,
                   hostDebugIntersectionError.data(),
                   _maxIntersectionSites * sizeof(float),
                   cudaMemcpyHostToDevice);
    }

    // TODO
    cudaMalloc(&_dDebugObsToModJs,
               depthWidth * depthHeight * 26 * sizeof(float3));
    ...
    {
        cudaMalloc(&_dPts->hostPtr()[i],
                   depthWidth * depthHeight * sizeof(DataAssociatedPoint));
    }
    ...
    for (int i = 0; i < _maxModels; ++i)
    {
        _iterationSummaries[i].resize(10);
    }

    std::cout << "max mem size:" << _maxDims + JTJSize(_maxDims) + 1
              << std::endl;
...
void Optimizer::generateObsSdfSplatZeroAndDistanceTransform(
    MirroredModel &model,
    const Observation &observation,
    const OptimizationOptions &opts)
{
    //    const float3 defaultOffset = model.getObsSdfoffset();
    //    const float4 sdfCenter =
    //    model.getTransformModelToCamera()*make_float4(defaultOffset.x,defaultOffset.y,defaultOffset.z,1);
...

    {
        ...
        if ((dim.x + 1) * (dim.y + 1) * (dim.z + 1) > maxDim)
        {
            ...
            cudaMalloc(&dZ,
                       (dim.x + 1) * (dim.y + 1) * (dim.z + 1) * sizeof(float));
            cudaMalloc(&dV, dim.x * dim.y * dim.z * sizeof(int));
        }

        distanceTransform3D<float, true>(
            model.getDeviceObsSdfData(), dTmp, dim.x, dim.y, dim.z, dZ, dV);

        cudaMemcpy(model.getDeviceObsSdfData(),
                   dTmp,
                   dim.x * dim.y * dim.z * sizeof(float),
                                              ...
                                              const OptimizationOptions &opts)
{
    ...
    computeTruncatedObsSdf(observation.dVertMap,
                           observation.width,
                           observation.height,
...
void Optimizer::computeObsToModContribution(Eigen::VectorXf &eJ,
                                            Eigen::MatrixXf &JTJ,
                                            float &error,
                                            ...
                                            const Observation &observation)
{
    ...
    cudaMemcpy(_result->hostPtr(),
               _result->devicePtr(),
               (dims + JTJSize(dims) + 1) * sizeof(float),
               cudaMemcpyDeviceToHost);
    CheckCudaDieOnError();
    if (pose.isReduced())
    {
        const LinearPoseReduction *reduction =
            static_cast<const LinearPoseReduction *>(pose.getReduction());
        if (reduction->isParamMap())
        {
            const ParamMapPoseReduction *paramMapReduction =
                static_cast<const ParamMapPoseReduction *>(reduction);
            normEqnsObsToModParamMap(pose.getDimensions(),
                                     ...
                                     _lastElements->hostPtr()[modelNum],
                                     _result->devicePtr());
            CheckCudaDieOnError();
            cudaMemcpy(_result->hostPtr(),
                       _result->devicePtr(),
    ...
    else
    {
        ...
        //        std::cout << "mem size:" << (dims + JTJSize(dims) + 1) <<
        //        std::endl;
    ...
    //    CheckCudaDieOnError();
    //    cudaMemcpy(_result->hostPtr(),_result->devicePtr(),(dims +
    //    JTJSize(dims) + 1)*sizeof(float),cudaMemcpyDeviceToHost);
    //    CheckCudaDieOnError();
    unpack(eJ, JTJ, error, _result->hostPtr(), opts.lambdaObsToMod, dims);
    CheckCudaDieOnError();
}

void Optimizer::computeModToObsContribution(Eigen::VectorXf &eJ,
                                            Eigen::MatrixXf &JTJ,
                                            float &error,
                                            const MirroredModel &model,
                                            const Pose &pose,
...
void Optimizer::computeSelfIntersectionContribution(
    Eigen::VectorXf &eJ,
    Eigen::MatrixXf &JTJ,
    float &error,
    const MirroredModel &model,
    ...
    const int debugOffset)
{
    ...
    if (pose.isReduced())
    {
        ...
        if (reduction->isParamMap())
        {
            const ParamMapPoseReduction *paramMapReduction =
                static_cast<const ParamMapPoseReduction *>(reduction);
            normEqnsSelfIntersectionParamMap(
                collisionCloud.devicePtr(),
        ...
        {
            normEqnsSelfIntersectionReduced(
                collisionCloud.devicePtr(),
                collisionCloud.length(),
                pose.getDimensions(),
                dims,
    ...
    else
    {
        normEqnsSelfIntersection(collisionCloud.devicePtr(),
                                 ...
                                 intersectionPotentialMatrix.devicePtr(),
                                 _result->devicePtr(),
                                 opts.debugIntersectionErr
                                     ? _dDebugIntersectionError + debugOffset
                                     : 0);
    }

    cudaMemcpy(_result->hostPtr(),
               _result->devicePtr(),
               ((dims - 6) + JTJSize((dims - 6)) + 1) * sizeof(float),
               cudaMemcpyDeviceToHost);
    ...
    unpack(eJtmp,
           ...
           error,
           _result->hostPtr(),
           opts.lambdaIntersection[modelNum + modelNum * nModels],
           (dims - 6));
...
void Optimizer::computeIntersectionContribution(
    Eigen::VectorXf &eJ,
    Eigen::MatrixXf &JTJ,
    float &error,
    ...
    const int debugOffset)
{
    ...
    const SE3 T_ds = dstModel.getTransformCameraToModel() *
                     srcModel.getTransformModelToCamera();
    ...
    if (pose.isReduced())
    {
        ...
        if (reduction->isParamMap())
        {
            const ParamMapPoseReduction *paramMapReduction =
                static_cast<const ParamMapPoseReduction *>(reduction);
            normEqnsIntersectionParamMap(collisionCloud.devicePtr(),
                                         collisionCloud.length(),
                                         pose.getDimensions(),
                                         dims,
                                         T_ds,
        ...
        {
            normEqnsIntersectionReduced(collisionCloud.devicePtr(),
                                        collisionCloud.length(),
                                        pose.getDimensions(),
                                        dims,
                                        T_ds,
    ...
    {
        normEqnsIntersection(collisionCloud.devicePtr(),
                             collisionCloud.length(),
                             dims,
                             T_ds,
                             T_sd,
    ...
    unpack(eJ,
           ...
           error,
           _result->hostPtr(),
           opts.lambdaIntersection[srcModelNum + dstModelNum * nModels],
           dims);
}
                             ...
                             const dart::PosePrior *prior)
{
    ...
    for (int iteration = 0; iteration < opts.numIterations; ++iteration)
    {
        ...
        if (predictionsNeeded)
        {
            ...
            _predictionRenderer->raytracePrediction(modelPtrs,
                                                    _depthPredictStream);
        ...
        if (opts.lambdaObsToMod > 0)
        {
            ...
            computeObsToModContribution(
                eJ, JTJ, obsToModError, model, pose, opts, observation);
        }
        if (opts.lambdaModToObs > 0)
        {
            computeModToObsContribution(
                eJ, JTJ, modToObsError, model, pose, T_obsSdf_camera, opts);
        }
        if (opts.lambdaIntersection[0] > 0)
        {
            computeSelfIntersectionContribution(eJ,
                                                JTJ,
                                                intersectionError,
                                                model,
                                                pose,
        ...
        JTJ += opts.regularization[0] *
               Eigen::MatrixXf::Identity(redDims, redDims);
        ...
        // compute next pose
        SE3 dT_mc = SE3Fromse3(se3(
            dalpha(0), dalpha(1), dalpha(2), dalpha(3), dalpha(4), dalpha(5)));
        ...
        for (int i = 0; i < pose.getArticulatedDimensions(); ++i)
        {
            if (!pose.isReduced())
            {
                pose.getReducedArticulation()[i] = std::min(
                    std::max(model.getJointMin(i),
                             pose.getReducedArticulation()[i] + dalpha(i + 6)),
                    model.getJointMax(i));
            }
            else
            {
                pose.getReducedArticulation()[i] = std::min(
                    std::max(pose.getReducedMin(i),
                             pose.getReducedArticulation()[i] + dalpha(i + 6)),
                    pose.getReducedMax(i));
            }
        }
...
void Optimizer::optimizePoses(
    std::vector<MirroredModel *> &models,
    ...
    std::map<int, PosePrior> &posePriors)
{
    ...
    if (nModels > _maxModels)
    {
        ...
        {
            cudaMalloc(&_dPts->hostPtr()[i],
                       width * height * sizeof(DataAssociatedPoint));
        }
        _dPts->syncHostToDevice();
    ...
    memcpy(planeOffsets.hostPtr(),
           opts.planeOffset.data(),
           nModels * sizeof(float));
    memcpy(planeNormals.hostPtr(),
           opts.planeNormal.data(),
           nModels * sizeof(float3));
    memcpy(distanceThresholds.hostPtr(),
           opts.distThreshold.data(),
           nModels * sizeof(float));
    ...
    for (int iteration = 0; iteration < opts.numIterations; ++iteration)
    {
        ...
        if (predictionsNeeded)
        {
            ...
            _predictionRenderer->raytracePrediction(constModelPtrs,
                                                    _depthPredictStream);
        ...
        errorAndDataAssociationMultiModel(
            ...
            sdfs.devicePtr(),
            nSdfs.devicePtr(),
            distanceThresholds.devicePtr(),
            normalThresholds.devicePtr(),
            planeOffsets.devicePtr(),
            planeNormals.devicePtr(),
            _lastElements->devicePtr(),
            _dPts->devicePtr(),
            opts.debugObsToModDA ? _dDebugDataAssocObsToMod : 0,
            opts.debugObsToModErr ? _dDebugObsToModError : 0,
        ...
        _lastElements
            ->syncDeviceToHost();  // needed in compute obs to mod contribution
                                   //        for (int m=0; m<nModels; ++m) {
        //            std::cout << _lastElements->hostPtr()[m] << " points
        ...
        {
            initDebugIntersectionError(_dDebugIntersectionError,
                                       _maxIntersectionSites);
        ...
        for (int m = 0; m < nModels; ++m)
        {
            ...
            if (opts.lambdaObsToMod > 0)
            {
                computeObsToModContribution(
                    eJ,
                    JTJ,
                    _iterationSummaries[m][iteration].errObsToMod,
                    model,
                    pose,
                    opts,
                    observation);
            ...
            if (opts.lambdaModToObs > 0)
            {
                computeModToObsContribution(
                    eJ,
                    JTJ,
                    _iterationSummaries[m][iteration].errModToObs,
                    model,
                    pose,
                    T_obsSdfs_camera[m],
                    opts);
            ...
            {
                computeSelfIntersectionContribution(
                    eJ,
                    JTJ,
                    intersectionError,
                    model,
                    ...
                    opts,
                    *collisionClouds[m],
                    *intersectionPotentialMatrices[m],
                    nModels,
            ...
            for (int d = 0; d < nModels; ++d)
            {
                ...
                {
                    computeIntersectionContribution(eJ,
                                                    JTJ,
                                                    intersectionError,
                                                    model,
                                                    *models[d],
            ...
            // ensure JTJ is full rank
            JTJ +=
                opts.regularization[m] *
                Eigen::MatrixXf::Identity(reducedDimensions, reducedDimensions);
            ...
            if (prior_it != posePriors.end() && articulation_dimension > 0)
            {
                ...
                pose_prior.bottomRows(articulation_dimension) =
                    Eigen::Map<Eigen::VectorXf>(posePrior.getPose(),
                                                articulation_dimension);

                prior_lambdas.bottomRows(articulation_dimension) =
                    Eigen::Map<Eigen::VectorXf>(posePrior.getWeights(),
                                                articulation_dimension);

                pose_estimate.bottomRows(articulation_dimension) =
                    Eigen::Map<Eigen::VectorXf>(pose.getReducedArticulation(),
                                                articulation_dimension);
            ...
            for (int i = 0; i < reducedDimensions; ++i)
            {
                for (int j = i; j < reducedDimensions; ++j)
                {
                    if (JTJ(i, j) != 0)
                    {
                        sparseJTJ.coeffRef(modelOffsets[m] + i,
                                           modelOffsets[m] + j) = JTJ(i, j);
                    }
                }
            }
        ...
        if (opts.lambdaModToObs > 0)
        {
            ...
            for (int m = 0; m < nModels; ++m)
            {
                _iterationSummaries[m][iteration].nPredictedPoints =
                    _lastElements->hostPtr()[m];
            }
        }
        ...
        {
            priors[p]->computeContribution(sparseJTJ,
                                           fullJTe,
                                           modelOffsets,
                                           priorOffsets[p],
                                           models,
        ...
        for (int m = 0; m < nModels; ++m)
        {
            ...
            SE3 dT_mc = SE3Fromse3(se3(paramUpdate(modelOffsets[m] + 0),
                                       paramUpdate(modelOffsets[m] + 1),
                                       paramUpdate(modelOffsets[m] + 2),
                                       paramUpdate(modelOffsets[m] + 3),
                                       paramUpdate(modelOffsets[m] + 4),
                                       paramUpdate(modelOffsets[m] + 5)));
            ...
            for (int i = 0; i < pose.getReducedArticulatedDimensions(); ++i)
            {
                if (!pose.isReduced())
                {
                    pose.getReducedArticulation()[i] = std::min(
                        std::max(model.getJointMin(i),
                                 pose.getArticulation()[i] +
                                     paramUpdate(modelOffsets[m] + i + 6)),
                        model.getJointMax(i));
                }
                else
                {
    ...
    if (opts.debugJTJ)
    {
        ...
        memcpy(JTJdata.hostPtr(),
               denseJTJ.data(),
               JTJdata.length() * sizeof(float));
        JTJdata.syncHostToDevice();
        visualizeMatrix(JTJdata.devicePtr(),
                        sysSize,
                        sysSize,
                        _JTJimg->devicePtr(),
                        320,
                        240,
                        make_uchar3(100, 0, 100),
                        0.0f,
                        500.0f);
