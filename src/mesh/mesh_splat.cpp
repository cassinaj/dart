...
void splatSolidMesh(const dart::Mesh& mesh, dart::Grid3D<float>& sdf)
{
    ...
    std::list<int>* candidateFaces =
        new std::list<int>[sdf.dim.x * sdf.dim.y * sdf.dim.z];
    ...
    for (int f = 0; f < mesh.nFaces; f++)
    {
        ...
        for (int z =
                 std::max(0.0, floor((min_z - sdf.offset.z) / sdf.resolution));
             z < std::min((double)sdf.dim.z,
                          ceil((max_z - sdf.offset.z) / sdf.resolution));
             z++)
        {
            for (int y = std::max(
                     0.0, floor((min_y - sdf.offset.y) / sdf.resolution));
                 y < std::min((double)sdf.dim.y,
                              ceil((max_y - sdf.offset.y) / sdf.resolution));
                 y++)
            {
                for (int x = std::max(
                         0.0, floor((min_x - sdf.offset.x) / sdf.resolution));
                     x <
                     std::min((double)sdf.dim.x,
                              ceil((max_x - sdf.offset.x) / sdf.resolution));
                     x++)
                {
                    candidateFaces[x + y * sdf.dim.x +
                                   z * sdf.dim.x * sdf.dim.y]
                        .push_back(f);
                }
            }
        }
    ...
    for (int z = 0; z < sdf.dim.z; z++)
    {
        for (int y = 0; y < sdf.dim.y; y++)
        {
            ...
            for (int x = 0; x < sdf.dim.x; x++)
            {
                ...
                const float3 r0 =
                    make_float3(x * sdf.resolution + sdf.offset.x,
                                (y + 0.5) * sdf.resolution + sdf.offset.y,
                                (z + 0.5) * sdf.resolution + sdf.offset.z);
                const float3 r1 =
                    make_float3((x + 1) * sdf.resolution + sdf.offset.x,
                                (y + 0.5) * sdf.resolution + sdf.offset.y,
                                (z + 0.5) * sdf.resolution + sdf.offset.z);

                for (std::list<int>::iterator it = candidates.begin();
                     it != candidates.end();
                     it++)
                {
                    const int f = *it;
                    ...
                    if (intersecting_faces.find(f) != intersecting_faces.end())
                    {
                        continue;
                    }
                    ...
                    const float st_denom =
                        dot(u, v) * dot(u, v) - dot(u, u) * dot(v, v);
                    const float s =
                        (dot(u, v) * dot(w, v) - dot(v, v) * dot(w, u)) /
                        st_denom;
                    const float t =
                        (dot(u, v) * dot(w, u) - dot(u, u) * dot(w, v)) /
                        st_denom;

                    if (s < -halo || t < -halo || s + t > (1 + halo))
                    {
                        continue;
                    }
    ...
    for (int z = 0; z < sdf.dim.z; z++)
    {
        for (int x = 0; x < sdf.dim.x; x++)
        {
            ...
            for (int y = 0; y < sdf.dim.y; y++)
            {
                ...
                const float3 r0 =
                    make_float3((x + 0.5) * sdf.resolution + sdf.offset.x,
                                y * sdf.resolution + sdf.offset.y,
                                (z + 0.5) * sdf.resolution + sdf.offset.z);
                const float3 r1 =
                    make_float3((x + 0.5) * sdf.resolution + sdf.offset.x,
                                (y + 1) * sdf.resolution + sdf.offset.y,
                                (z + 0.5) * sdf.resolution + sdf.offset.z);
        ...
        for (int x = 0; x < sdf.dim.x; x++)
        {
            ...
            for (int z = 0; z < sdf.dim.z; z++)
            {
                ...
                const float3 r0 =
                    make_float3((x + 0.5) * sdf.resolution + sdf.offset.x,
                                (y + 0.5) * sdf.resolution + sdf.offset.y,
                                z * sdf.resolution + sdf.offset.z);
                const float3 r1 =
                    make_float3((x + 0.5) * sdf.resolution + sdf.offset.x,
                                (y + 0.5) * sdf.resolution + sdf.offset.y,
                                (z + 1) * sdf.resolution + sdf.offset.z);
