...
class Model
{
  public:
    ...
    void renderSkeleton(const float sphereSize = 0.01,
                        const float lineSize = 4) const;
    ...
    const inline SE3 getTransformCameraToFrame(const int frame) const
    {
        return getTransformModelToFrame(frame) * getTransformCameraToModel();
    }
    const inline SE3 getTransformFrameToCamera(const int frame) const
    {
        return getTransformModelToCamera() * getTransformFrameToModel(frame);
    }

    virtual int getFrameParent(const int frame) const = 0;
    inline int getFrameSdfNumber(const int frame) const
    {
        return _frameSdfNumbers[frame];
    }
    ...
    inline GeomType getGeometryType(const int geomNumber) const
    {
        return _geomTypes[geomNumber];
    }
    virtual uchar3 getGeometryColor(const int geomNumber) const = 0;
    const inline float3 getGeometryScale(const int geomNumber) const
    {
        return _geomScales[geomNumber];
    }
    const inline SE3 getGeometryTransform(const int geomNumber) const
    {
        return _geomTransforms[geomNumber];
    }
    ...
    inline const Mesh &getMesh(const int meshNumber) const
    {
        return _renderer->getMesh(meshNumber);
    }
    ...
    void getModelJacobianOfModelPoint(const float4 &modelPoint,
                                      const int associatedFrame,
                                      std::vector<float3> &J3D) const;

    inline float getJointMin(const int joint) const
    {
        return _jointLimits[joint].x;
    }
    inline float getJointMax(const int joint) const
    {
        return _jointLimits[joint].y;
    }

    const std::string &getJointName(const int joint) const
    {
        return _jointNames[joint];
    }
    void renderSdf(const dart::Grid3D<float> &sdf, float levelSet) const;

    void getArticulatedBoundingBox(float3 &mins,
                                   float3 &maxs,
                                   const float modelSdfPadding,
                                   const int nSweepPoints = 4);
    ...
    void setModelVersion(const int modelVersion)
    {
  ...
  protected:
    ...
    inline void setMeshNumber(const int geomNumber, const int meshNumber)
    {
        _meshNumbers[geomNumber] = meshNumber;
    }
  ...
  private:
    ...
    void renderGeometries(void (Model::*prerenderFunc)(const int,
                                                       const int,
                                                       const char *) const,
                          const char *args = 0,
                          void (Model::*postrenderFunc)() const = 0) const;

    void renderColoredPrerender(const int frameNumber,
                                const int geomNumber,
                                const char *args) const;

    void renderWireframePrerender(const int frameNumber,
                                  const int geomNumber,
                                  const char *args) const;
    void renderWireFramePostrender() const;

    void renderLabeledPrerender(const int frameNumber,
                                const int geomNumber,
                                const char *args) const;

    // frame-level rendering
    void renderFrames(void (Model::*renderFrameFunc)(const int, const char *)
                          const,
