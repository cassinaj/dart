...
void Model::shutdownRenderer() { delete Model::_renderer; }
void Model::render(const char alpha) const
{
    renderGeometries(&Model::renderColoredPrerender, (const char*)&alpha);
}

void Model::renderColoredPrerender(const int frameNumber,
                                   const int geomNumber,
...
{
    renderGeometries(
        &Model::renderWireframePrerender, 0, &Model::renderWireFramePostrender);
}
...
void Model::renderWireframePrerender(const int frameNumber,
                                     const int geomNumber,
                                     const char* args) const
{
...
void Model::renderLabeledPrerender(const int frameNumber,
                                   const int geomNumber,
                                   const char* args) const
{
...
void Model::renderGeometries(void (Model::*prerenderFunc)(const int,
                                                          const int,
                                                          const char*) const,
                             const char* args,
    ...
    for (int f = 0; f < getNumFrames(); f++)
    {
        ...
        if (getFrameNumGeoms(f) > 0)
        {
            ...
            for (int g = 0; g < getFrameNumGeoms(f); g++)
            {
                ...
                glPushMatrix();
                const dart::SE3 geomT = getGeometryTransform(
                    geomNumber);  // dart::SE3Fromse3(dart::se3(&geomParams[3]));
                float mxData[16] = {geomT.r0.x,
                                    geomT.r1.x,
                                    geomT.r2.x,
                                    0,
                                    geomT.r0.y,
...
void Model::renderSdf(const dart::Grid3D<float>& sdf, float levelSet) const
{
    ...
    for (int z = 0; z < sdf.dim.z; z++)
    {
        for (int y = 0; y < sdf.dim.y; y++)
        {
            ...
            {
                if (sdf.data[x + y * sdf.dim.x + z * sdf.dim.x * sdf.dim.y] <=
                    levelSet)
                {
                    glPushMatrix();

                    glTranslated(sdf.offset.x + (x + 0.5) * sdf.resolution,
                                 sdf.offset.y + (y + 0.5) * sdf.resolution,
                                 sdf.offset.z + (z + 0.5) * sdf.resolution);
    ...
    glVertex3d(
        sdf.offset.x, sdf.offset.y, sdf.offset.z + sdf.dim.z * sdf.resolution);

    glVertex3d(
        sdf.offset.x, sdf.offset.y + sdf.dim.y * sdf.resolution, sdf.offset.z);
    glVertex3d(sdf.offset.x,
               sdf.offset.y + sdf.dim.y * sdf.resolution,
               sdf.offset.z + sdf.dim.z * sdf.resolution);

    glVertex3d(
        sdf.offset.x + sdf.dim.x * sdf.resolution, sdf.offset.y, sdf.offset.z);
    glVertex3d(sdf.offset.x + sdf.dim.x * sdf.resolution,
               sdf.offset.y,
               sdf.offset.z + sdf.dim.z * sdf.resolution);
...
void Model::renderFrames(void (Model::*renderFrameFunc)(const int, const char*)
                             const,
                         const char* args) const
{
    glPushMatrix();
...
void Model::getModelJacobianOfModelPoint(const float4& modelPoint,
                                         const int frame,
                                         std::vector<float3>& J3D) const
{
    ...
    //    std::cout << "-=-=-=-=-=-=-=-=-=-=-" << std::endl;
    //    std::cout << "modelPoint: " << modelPoint.x << ", " << modelPoint.y <<
    //    ", " << modelPoint.z << std::endl << std::endl;
    ...
    for (int joint = 0; joint < getNumJoints(); ++joint)
    {
        ...
        SE3 T_pa = SE3FromTranslation(jointPosition) *
                   SE3Fromse3(se3(0,
                                  0,
                                  0,
                                  jointOrientation.x,
        ...
        {
            // get the point representing link's origin relative to the joint
            // link's parent's frame
            float4 parentLinkPoint = getTransformModelToFrame(jointParent) *
                                     modelPoint;  // parentT_mf*modelPoint;
            float4 axisRelativePoint =
                getTransformModelToFrame(jointFrame) * modelPoint;

            float3 derivative_a =
                cross(getJointAxis(joint), make_float3(axisRelativePoint));

            float3 derivative =
                SE3Rotate(getTransformFrameToModel(jointFrame), derivative_a);

            //            std::cout << "joint " << joint << ": " << std::endl;
            //            std::cout << "\taxisRelativePoint = " <<
            //            axisRelativePoint.x << ", " << axisRelativePoint.y <<
            //            ", " << axisRelativePoint.z << std::endl;
            //            std::cout << "\tderivative_a = " << derivative_a.x <<
            //            ", " << derivative_a.y << ", " << derivative_a.z <<
            //            std::endl;
            //            std::cout << "\tderivative = " << derivative.x << ", "
            //            << derivative.y << ", " << derivative.z << std::endl;
            //            std::cout << std::endl << std::endl;

            //            // convert the point to a vector and rotate it into
            //            the frame of joint link's parent
            //            parentLinkPoint.w = 0;
            //            float4 pointVec = parentT_mf*parentLinkPoint;

            //            // take cross-produce of z vector and vector from
            //            joint link's parent to point
            //            float3 derivative =
            //            -1*cross(axis_m,make_float3(pointVec));
        ...
        else if (jointType == PrismaticJoint)
        {
            ...
            const float3 axis_m =
                SE3Rotate(getTransformFrameToModel(jointParent), axis_p);
...
void Model::getArticulatedBoundingBox(float3& mins,
                                      float3& maxs,
                                      const float modelSdfPadding,
                                      const int nSweepPoints)
{
    mins = getSdf(0).offset + make_float3(modelSdfPadding);
    maxs = getSdf(0).offset - make_float3(modelSdfPadding) +
           getSdf(0).resolution *
               make_float3(getSdf(0).dim.x, getSdf(0).dim.y, getSdf(0).dim.z);
    ...
    for (int s = 1; s < getNumSdfs(); ++s)
    {
        ...
        if (jointDependencies.size() == 0)
        {
            continue;
        }

        float4 sdfMin_f =
            make_float4(sdf.offset + make_float3(modelSdfPadding), 1);
        float4 sdfMax_f = make_float4(
            sdf.offset - make_float3(modelSdfPadding) +
                sdf.resolution * make_float3(sdf.dim.x, sdf.dim.y, sdf.dim.z),
            1);
        ...
        for (int j = 0; j < jointDependencies.size(); ++j)
        {
            ...
            {
                const std::vector<float>& combinations =
                    jointAngleCombinations[c];
                for (int a = 0; a < nSweepPoints; ++a)
                {
                    float jointAngle = jointMin +
                                       ((float)a / (nSweepPoints - 1.f)) *
                                           (jointMax - jointMin);
        ...
        //        std::cout << jointDependencies.size() << " joint dependencies
        //        leads to " << jointAngleCombinations.size() <<
