    ...
    : Model(copy)
{
    ...
    memcpy(_T_mf->hostPtr(),
           copy.getTransformsFrameToModel(),
           numFrames * sizeof(SE3));
    _T_mf->syncHostToDevice();

    _T_fm = new MirroredVector<SE3>(numFrames);
    memcpy(_T_fm->hostPtr(),
           copy.getTransformsModelToFrame(),
           numFrames * sizeof(SE3));
    ...
    if (numSdfs > 0)
    {
        ...
        _sdfs = new MirroredGrid3DVector<float>(
            numSdfs, (Grid3D<float> *)copy.getSdfs());
    ...
    else
    {
        ...
        _sdfs = 0;
        _sdfColors = 0;
        std::cerr << "warning: you've created a MirroredModel with no SDFs. "
                     "Did you forget to call voxelize?"
                  << std::endl;
    }
    ...
    memcpy(_dependencies->hostPtr(),
           copy.getDependencies(),
           numFrames * numJoints * sizeof(int));
    ...
    memset(_obsSdf->hostData(),
           0,
           obsSdfDim.x * obsSdfDim.y * obsSdfDim.z * sizeof(float));
}
    ...
    for (int f = 1; f < getNumFrames(); ++f)
    {
        ...
        switch (_jointTypes->hostPtr()[joint])
        {
            case RotationalJoint:
                T_pf =
                    T_pf * SE3Fromse3(se3(0,
                                          0,
                                          0,
                                          p * _jointAxes->hostPtr()[joint].x,
                                          p * _jointAxes->hostPtr()[joint].y,
                                          p * _jointAxes->hostPtr()[joint].z));
                ++j;
                break;
            case PrismaticJoint:
                T_pf = T_pf * SE3Fromse3(se3(p * _jointAxes->hostPtr()[joint].x,
                                             p * _jointAxes->hostPtr()[joint].y,
                                             p * _jointAxes->hostPtr()[joint].z,
                                             0,
                                             0,
...
void MirroredModel::setPose(const Pose &pose)
{
    ...
    for (int f = 1; f < getNumFrames(); ++f)
    {
        float p = std::min(std::max(getJointMin(j), pose.getArticulation()[j]),
                           getJointMax(j));
