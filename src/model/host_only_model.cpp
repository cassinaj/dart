...
int HostOnlyModel::addFrame(int parent,
                            JointType type,
                            std::string posX,
                            std::string posY,
                            std::string posZ,
                            ...
                            std::string mimicOffset)
{
    ...
    for (int f = 0; f < frameNum; ++f)
    {
        _dependencies.insert(
            _dependencies.begin() + ((f + 1) * getNumJoints() - 1), 1, 0);
    }
    ...
    _T_mf.push_back(SE3Fromse3(se3(pos.x, pos.y, pos.z, 0, 0, 0)) *
                    dart::SE3Fromse3(dart::se3(
                        0, 0, 0, orientation.x, orientation.y, orientation.z)));
    _T_fm.push_back(SE3Invert(_T_mf[_nFrames]));

    if (_modelVersion == 0)
    {
        _T_pf.push_back(
            SE3Fromse3(se3(pos.x, pos.y, pos.z, 0, 0, 0)) *
            SE3Fromse3(
                se3(0, 0, 0, orientation.x, orientation.y, orientation.z)));
    }
    else
    {
        _T_pf.push_back(SE3FromTranslation(make_float3(pos.x, pos.y, pos.z)) *
                        SE3FromEuler(make_float3(
                            orientation.z, orientation.y, orientation.x)));
    }
    _nFrames++;

    jointNameIndexMap[jointName] = getNumJoints() - 1;
...
void HostOnlyModel::addGeometry(int frame,
                                ...
                                const std::string meshFilename)
{
    ...
    if (geometryType == MeshType)
    {
        if (_renderer == 0)
        {
            std::cerr << "model renderer has no mesh reader; mesh models may "
                         "not be used"
                      << std::endl;
            return;
        }
    ...
    {
        _geomTransforms.push_back(SE3FromTranslation(tx, ty, tz) *
                                  SE3FromEuler(make_float3(rz, ry, rx)));
    }
...
void HostOnlyModel::computeStructure()
{
    for (int f = 0; f < getNumFrames() - 1; ++f)
    {
        ...
        _jointLimits[f] =
            make_float2(evaluateExpression(_frameParamExpressions[f][9]),
    ...
    for (int joint = 0; joint < getNumJoints(); ++joint)
    {
        if (_modelVersion == 0)
        {
            _T_pf[joint] = SE3Fromse3(se3(_positions[joint].x,
                                          _positions[joint].y,
                                          _positions[joint].z,
                                          0,
                                          0,
                           ...
                           SE3Fromse3(se3(0,
                                          0,
                                          0,
                                          _orientations[joint].x,
                                          _orientations[joint].y,
                                          _orientations[joint].z));
        }
        else
        {
            _T_pf[joint] =
                SE3FromTranslation(make_float3(_positions[joint].x,
                                               _positions[joint].y,
                                               _positions[joint].z)) *
                SE3FromEuler(make_float3(_orientations[joint].z,
                                         _orientations[joint].y,
                                         _orientations[joint].x));
        }
        if (!mimicJoints[joint].empty())
        {
            auto sourceJointIt = jointNameIndexMap.find(mimicJoints[joint]);
            if (sourceJointIt == jointNameIndexMap.end())
            {
                std::cerr << "Mimic joint " << _jointNames[joint]
                          << " is referencing an invalid joint "
                          << mimicJoints[joint] << std::endl;
                exit(1);
            }
            mimicJointMap[joint] = sourceJointIt->second;
        }
    }
    ...
    {
        float3 scale =
            make_float3(evaluateExpression(_geomParamExpressions[g][0]),
                        evaluateExpression(_geomParamExpressions[g][1]),
                        evaluateExpression(_geomParamExpressions[g][2]));
        _geomScales[g] = scale;
        if (_modelVersion == 0)
        {
            _geomTransforms[g] = SE3Fromse3(
                se3(evaluateExpression(_geomParamExpressions[g][3]),
        ...
        else
        {
            _geomTransforms[g] =
                SE3FromTranslation(
                    evaluateExpression(_geomParamExpressions[g][3]),
                    evaluateExpression(_geomParamExpressions[g][4]),
                    evaluateExpression(_geomParamExpressions[g][5])) *
                SE3FromEuler(make_float3(
                    evaluateExpression(_geomParamExpressions[g][8]),
...
// TODO
void HostOnlyModel::voxelize(float resolution,
                             float padding,
                             std::string cacheFile)
{
    ...
    for (int i = 0; i < _nFrames; i++)
    {
        ...
        std::string filename = dart::stringFormat(
            "%s.sdf%02d.res%06f.sdf", cacheFile.c_str(), n, resolution);
        struct stat buffer;

        _sdfColors.push_back(
            getGeometryColor(getFrameGeoms(i)[getFrameNumGeoms(i) - 1]));
        ...
        if (cacheFile != "" && stat(filename.c_str(), &buffer) == 0)
        {
            ...
            const int sdfSize =
                _sdfs[n].dim.x * _sdfs[n].dim.y * _sdfs[n].dim.z;
        ...
        else
        {
...
#if CUDA_BUILD
            float *sdfIn;
            cudaMalloc(&sdfIn, nVoxels * sizeof(float));
            float *sdfOut;
            cudaMalloc(&sdfOut, nVoxels * sizeof(float));
            cudaMemcpy(sdfIn,
                       _sdfs[n].data,
            ...
            signedDistanceTransform3D<float, true>(
                sdfIn, sdfOut, _sdfs[n].dim.x, _sdfs[n].dim.y, _sdfs[n].dim.z);

#if CUDA_BUILD
            cudaMemcpy(_sdfs[n].data,
                       sdfOut,
                       nVoxels * sizeof(float),
...
void HostOnlyModel::voxelize2(float resolution,
                              float padding,
                              std::string cacheFile)
{
    _frameSdfNumbers.clear();
    ...
    for (int i = 0; i < _nFrames; i++)
    {
        ...
        else
        {
            ...
            for (int k = 0;
                 k < _sdfs[n].dim.x * _sdfs[n].dim.y * _sdfs[n].dim.z;
                 ++k)
            {
                _sdfs[n].data[k] = 1e20;
            }
            for (int g = 0; g < getFrameNumGeoms(i); ++g)
            {
                ...
                switch (getGeometryType(geomNum))
                {
                    case MeshType:
                    {
                        ...
                        transformMesh(transformedMesh,
                                      getGeometryTransform(geomNum));
                    ...
                    {
                        std::cout << "computing analytic sphere SDF"
                                  << std::endl;
                        ...
                        analyticSphereSdf(
                            _sdfs[n], getGeometryTransform(geomNum), radius);
                    }
                    break;
                }
            }
            //            for (int k=0;
            //            k<_sdfs[n].dim.x*_sdfs[n].dim.y*_sdfs[n].dim.z; ++k) {
            //                float & dist = _sdfs[n].data[k];
            //                //dist = sqrt(dist/resolution);
            //                dist = signs[k] == 0 ? -sqrt(dist)/resolution :
            //                sqrt(dist)/resolution;
            //            }
...
void HostOnlyModel::setGeometryColor(int geomNumber,
                                     unsigned char red,
                                     unsigned char green,
                                     unsigned char blue)
{
...
void HostOnlyModel::setJointLimits(const int joint,
                                   const float min,
                                   const float max)
{
...
void HostOnlyModel::setArticulation(const float *pose)
{
    ...
    {
        const float p =
            std::min(std::max(getJointMin(j - 6), pose[j]), getJointMax(j - 6));
        ...
        switch (_jointTypes[joint])
        {
            case RotationalJoint:
                ...
                break;
            case PrismaticJoint:
                T_pf = T_pf * SE3Fromse3(se3(p * _axes[joint].x,
                                             p * _axes[joint].y,
                                             p * _axes[joint].z,
                                             0,
                                             0,
...
void HostOnlyModel::setPose(const Pose &pose)
{
    ...
    {
        const float p =
            std::min(std::max(getJointMin(j), pose.getArticulation()[j]),
                     getJointMax(j));
...
void HostOnlyModel::voxelizeFrame(Grid3D<float> &sdf,
                                  const int frame,
                                  const float fg,
                                  const float bg,
                                  ...
                                  const float Rc)
{
    ...
    for (int g = 0; g < getFrameNumGeoms(frame); g++)
    {
        ...
        switch (getGeometryType(geom))
        {
            ...
            case MeshType:
                ...
                SE3 T = SE3Fromse3(se3(0, 0, 0, p[6], p[7], p[8])) *
                        SE3Fromse3(se3(p[3], p[4], p[5], 0, 0, 0));

                transformMesh(transformedMesh, getGeometryTransform(geom));

                //            for (int v=0; v<mesh.nVertices; ++v) {
                //                const float3 &vert = mesh.vertices[v];
                //                transformedMesh.vertices[v] =
                //                SE3Transform(T_SE3,make_float3(p[0]*vert.x,p[1]*vert.y,p[2]*vert.z));
                //                transformedMesh.normals[v] =
                //                SE3Rotate(T_SE3,mesh.normals[v]);
                //            }
    ...
    std::cout << " voxel dimensions are " << dim.x << ", " << dim.y << ", "
              << dim.z << std::endl;
            ...
            case PrimitiveSphereType:
                ...
                {  // ellipsoid is not rotated
                    ...
                    for (int z = 0; z < dim.z; z++)
                    {
                        float vox_z =
                            z * resolution +
                            off_z;  // voxel z position (in geometric units)
                        float C = vox_z * vox_z / (p[2] * p[2]);
                        for (int y = 0; y < dim.y; y++)
                        {
                            float vox_y =
                                y * resolution +
                                off_y;  // voxel y position (in geometric units)
                            float B = vox_y * vox_y / (p[1] * p[1]);
                            for (int x = 0; x < dim.x; x++)
                            {
                                float vox_x = x * resolution +
                                              off_x;  // voxel x position (in
                                                      // geometric units)
                ...
                {  // ellipsoid is rotated
                    ...
                    T_vox_geom =
                        SE3Fromse3(se3(p[3], p[4], p[5], p[6], p[7], p[8]));
                ...
                if (p[6] == 0 && p[7] == 0 && p[8] == 0)
                {
                    for (int z = 0; z < dim.z; z++)
                    {
                        float vox_z =
                            z * resolution +
                            off_z;  // voxel z position (in geometric units)
                        if (vox_z > 0.5 * p[2] || vox_z < -0.5 * p[2]) continue;
                        for (int y = 0; y < dim.y; y++)
                        {
                            float vox_y =
                                y * resolution +
                            ...
                            for (int x = 0; x < dim.x; x++)
                            {
                                ...
                                if (vox_x > 0.5 * p[0] || vox_x < -0.5 * p[0])
                                    continue;
                                sdf.data[x + y * dim.x + z * dim.x * dim.y] =
                                    fg;
                            }
                        }
                    }
