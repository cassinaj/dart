...
class PoseReduction
{
  public:
    ...
    virtual void projectReducedToFull(const float* reduced,
                                      float* full) const = 0;
    ...
    inline const std::string& getName(const int dim) const
    {
        return _names[dim];
    }
...
class LinearPoseReduction : public PoseReduction
{
  public:
    ...
    inline float getFirstDerivative(const int reducedDim, const int fullDim)
    {
        return _A[reducedDim + fullDim * _redDims];
    }
...
{
  public:
    ParamMapPoseReduction(const int fullDims,
                          const int redDims,
                          const int* mapping,
                          float* mins,
                          float* maxs,
...
{
  public:
    NullReduction(const int dims,
                  const float* mins,
                  const float* maxs,
                  const std::string* names)
        : PoseReduction(dims, dims)
    {
