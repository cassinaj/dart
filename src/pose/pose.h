...
class Pose
{
  public:
    ...
    {
        return _reduction->getFullDimensions() + 6;
    }

    inline int getArticulatedDimensions() const
    {
        return _reduction->getFullDimensions();
    }

    inline int getReducedDimensions() const
    {
        return _reduction->getReducedDimensions() + 6;
    }

    inline int getReducedArticulatedDimensions() const
    {
        return _reduction->getReducedDimensions();
    }
    ...
    inline const float* getReducedArticulation() const
    {
        return _reducedArticulation;
    }

    virtual void projectReducedToFull()
    {
        return _reduction->projectReducedToFull(_reducedArticulation,
                                                _fullArticulation);
    }

    inline const float* getFirstDerivatives() const
    {
        return _reduction->getFirstDerivatives();
    }

    inline const float* getDeviceFirstDerivatives() const
    {
        return _reduction->getDeviceFirstDerivatives();
    }

    inline float getReducedMin(const int dim) const
    {
        return _reduction->getMin(dim);
    }

    inline float getReducedMax(const int dim) const
    {
        return _reduction->getMax(dim);
    }

    inline const std::string& getReducedName(const int dim) const
    {
        return _reduction->getName(dim);
    }
    ...
    inline void setTransformCameraToModel(const SE3 T_mc)
    {
        _T_mc = T_mc;
        _T_cm = SE3Invert(T_mc);
    }

    inline void setTransformModelToCamera(const SE3 T_cm)
    {
        _T_cm = T_cm;
        _T_mc = SE3Invert(T_cm);
    }
