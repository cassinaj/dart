                       ...
                       const bool cacheSdfs)
{
    ...
    const std::string modelName = filename.substr(
        substrStart, diff > 0 ? diff : filename.size() - substrStart);
    std::cout << "model name: " << modelName << std::endl;

    //    // TODO
    //    if (model.getNumGeoms() < 2) {
    //        model.voxelize2(modelSdfResolution,modelSdfPadding,cacheSdfs ?
    //        dart::stringFormat("model%02d",_mirroredModels.size()) : "");
    //    } else {
    model.voxelize(
        modelSdfResolution,
        modelSdfPadding,
        cacheSdfs ? dart::stringFormat("/tmp/%s", modelName.c_str()) : "");
    //    }
    ...
    if (obsSdfResolution <= 0)
    {
        ...
        if (model.getPoseDimensionality() == 6)
        {
            ...
            float3 rootSdfSizeMeters =
                rootSdf.resolution *
                make_float3(rootSdf.dim.x, rootSdf.dim.y, rootSdf.dim.z);
            ...
            float maxDimMeters =
                std::max(std::max(obsSdfSizeMeters.x, obsSdfSizeMeters.y),
                         obsSdfSizeMeters.z);
        ...
        else
        {
            ...
            float3 obsSdfSizeMeters =
                max - min + 2 * make_float3(obsSdfPadding);
            float maxDimMeters =
                std::max(std::max(obsSdfSizeMeters.x, obsSdfSizeMeters.y),
                         obsSdfSizeMeters.z);
            obsSdfResolution = maxDimMeters / obsSdfSize;
            obsSdfOffset =
                min + 0.5 * (obsSdfSizeMeters - make_float3(maxDimMeters)) -
                make_float3(obsSdfPadding);
        }
    }

    _mirroredModels.push_back(new MirroredModel(
        model, make_uint3(obsSdfSize), obsSdfResolution, obsSdfOffset));
    ...
    for (int f = 0; f < model.getNumFrames(); ++f)
    {
        ...
        if (sdfNum >= 0)
        {
            _collisionCloudSdfStarts[getNumModels() - 1][sdfNum] =
                nPointsCumulative;
        ...
        for (int g = 0; g < model.getFrameNumGeoms(f); ++g)
        {
            ...
            switch (model.getGeometryType(gNum))
            {
                ...
                default:
                {
                    std::cerr << "collision clouds for type "
                              << model.getGeometryType(gNum)
            ...
            delete samplerMesh;
            //            std::cout << "sampled " << sampledPoints.size() << "
            //            points" << std::endl;
            ...
            if (collisionCloud == 0)
            {
                start = 0;
                collisionCloud =
                    new MirroredVector<float4>(sampledPoints.size());
            }
            else
            {
            ...
            _collisionCloudSdfLengths[getNumModels() - 1][sdfNum] +=
                sampledPoints.size();
    ...
    MirroredVector<int> *intersectionPotentialMatrix =
        new MirroredVector<int>(model.getNumSdfs() * model.getNumSdfs());
    memset(intersectionPotentialMatrix->hostPtr(),
           0,
           intersectionPotentialMatrix->length() * sizeof(int));
    ...
    _T_fms->hostPtr()[m] = mm.getDeviceTransformsModelToFrame();
    _T_fms->syncHostToDevice();
    _sdfFrames->hostPtr()[m] = mm.getDeviceSdfFrames();
    _sdfFrames->syncHostToDevice();
    _sdfs->hostPtr()[m] = mm.getDeviceSdfs();
    _sdfs->syncHostToDevice();
    _nSdfs->hostPtr()[m] = mm.getNumSdfs();
    _nSdfs->syncHostToDevice();
    ...
    *_dampingMatrices.back() = Eigen::MatrixXf::Zero(reducedDims, reducedDims);
    ...
    if (getNumModels() > 1)
    {
        _opts.distThreshold.resize(getNumModels());
        _opts.distThreshold.back() = _opts.distThreshold.front();
        _opts.regularization.resize(getNumModels());
        _opts.regularization.back() = _opts.regularization.front();
        _opts.regularizationScaled.resize(getNumModels());
        _opts.regularizationScaled.resize(getNumModels());
        _opts.planeOffset.resize(getNumModels());
        _opts.planeOffset.back() = _opts.planeOffset.front();
        _opts.planeNormal.resize(getNumModels());
        _opts.planeNormal.back() = _opts.planeNormal.front();
        _opts.lambdaIntersection.resize(getNumModels() * getNumModels());
        for (int i = (getNumModels() - 1) * (getNumModels() - 1);
             i < _opts.lambdaIntersection.size();
             ++i)
        {
...
bool Tracker::addDepthSource(DepthSource<DepthType, ColorType> *depthSource)
{
    if (_depthSource != 0)
    {
        std::cerr << "using multiple depth sources is not supported yet!"
                  << std::endl;
        return false;
    }

    _depthSource = depthSource;
    _pcSource = new PointCloudSource<DepthType, ColorType>(
        depthSource, make_float2(0.1, 10));  // settingrange

    _optimizer = new Optimizer(depthSource,
                               depthSource->getDepthWidth() / 2,
                               depthSource->getDepthHeight() / 2);

    return true;
}
template bool Tracker::addDepthSource<float, uchar3>(
    DepthSource<float, uchar3> *depthSource);
template bool Tracker::addDepthSource<float, uchar4>(
    DepthSource<float, uchar4> *depthSource);
template bool Tracker::addDepthSource<ushort, uchar3>(
    DepthSource<ushort, uchar3> *depthSource);
                          ...
                          const float3 obsSdfCenter)
{
    ...
    MirroredModel *newModel = new MirroredModel(
        model, make_uint3(obsSdfSize), obsSdfResolution, obsSdfCenter, modelID);
...
void Tracker::stepForward()
{
    if (!initialized())
    {
        std::cerr << "the tracker is not initialized properly for tracking. "
                     "make sure a depth source and a model have been added"
                  << std::endl;
        return;
    }
    ...
    // float4 cpoint = *(_pcSource->getHostVertMap() +
    // (_pcSource->getDepthHeight() / 2 * _pcSource->getDepthWidth() +
    // _pcSource->getDepthWidth() / 2));
    // std::cout << "*********central point " << cpoint.x << " " << cpoint.y <<
    // " " << cpoint.z << " " << "\n";
    // std::cout << "testing";
}
...
void Tracker::setIntersectionPotentialMatrix(const int modelNum, const int *mx)
{
    ...
    _intersectionPotentialMatrices[modelNum] =
        new MirroredVector<int>(nSdfs * nSdfs);
    memcpy(_intersectionPotentialMatrices[modelNum]->hostPtr(),
           mx,
           nSdfs * nSdfs * sizeof(int));
