...
/**
 * @brief MirroredData is a super class for various data types that are mirrored
...
class MirroredVector : public MirroredData
{
  public:
    /**
     * This will create a MirroredVector of the specified length, allocating the
    ...
    {
        cudaMemcpy(
            _dVector, _hVector, _length * sizeof(T), cudaMemcpyHostToDevice);
    }
    inline void syncDeviceToHost()
    {
        cudaMemcpy(
            _hVector, _dVector, _length * sizeof(T), cudaMemcpyDeviceToHost);
    }
...
MirroredVector<T>::MirroredVector(const uint length) : _length(length)
{
    if (length != 0)
    {
        CUDA_ASSERT(cudaMallocHost(&_hVector, length * sizeof(T)));
        CUDA_ASSERT(cudaMalloc(&_dVector, length * sizeof(T)));
    }
    else
    {
        _hVector = NULL;
...
void MirroredVector<T>::resize(const uint length)
{
    ...
    if (length != 0)
    {
        ...
        cudaMemcpy(_hVector,
                   hTmp,
                   std::min(_length, length) * sizeof(T),
                   cudaMemcpyHostToHost);
...
template <typename T>
/**
 * @brief MirroredGrid3D class provides a mirrored implementation of the Grid3D
 * class.
 */
class MirroredGrid3D : public MirroredData
{
  public:
    /**
     * @brief A MirroredGrid3D instance must be constructed from a stricly
     * host-resident Grid3D instance of the same templated type. The constructor
     * allocates all appropriate memory and copies the data from the given
     * Grid3D object to both device and host.
    ...
    /**
     * This returns a pointer directly to the linearized data of the
     * host-resident grid. It is equivalent to hostGrid()->data.
     * @return A pointer to the host-resident grid data.
     */
    inline T *hostData() { return _hData; }
    /**
     * This returns a pointer directly to the linearized data of the
     * device-resident grid. It is equivalent to deviceGrid()->data.
...
template <typename T>
MirroredGrid3D<T>::MirroredGrid3D(const Grid3D<T> &grid)
    : _dataSize(grid.dim.x * grid.dim.y * grid.dim.z * sizeof(T))
{
...
template <typename T>
/**
 * @brief The MirroredGrid3DVector class provides a deep-mirrored vector of
 * Grid3D instances. That is to say that the instances are mirrored, as well as
 * the vector of pointers to the grids.
 */
class MirroredGrid3DVector : public MirroredData
{
  public:
    /**
     * A MirroredGrid3DVector instance must be constructed from an array of
     * stricly host-resident Grid3D instances of the same templated type. The
     * constructor allocates all appropriate memory and copies the data from the
     * given Grid3D objects to both device and host.
     * @param length The number of grids to be mirrored.
     * @param grids An array of pointers to Grid3D instances of size length. The
     * constructor will allocate all appropriate memory and copy the data from
     * all grids to both device and host. The MirroredGrid3DVector instance does
     * not own these pointers.
     */
    ...
    inline void syncHostToDevice()
    {
        for (int i = 0; i < _length; ++i)
        {
            ...
            cudaMemcpy(
                _dDatas[i], _hDatas[i], _dataSizes[i], cudaMemcpyHostToDevice);
        }
        cudaMemcpy(_dGrids,
                   _hGrids,
                   _length * sizeof(Grid3D<T>),
    ...
    inline void syncDeviceToHost()
    {
        ...
        {
            cudaMemcpy(
                _hDatas[i], _dDatas[i], _dataSizes[i], cudaMemcpyDeviceToHost);
            _hGrids[i].data = _hDatas[i];
        }
    }

    /**
     * This returns a pointer to the first host-resident Grid3D instance. A
     * pointer to (host-resident) instance i (for i < length) can be accessed
     * via hostGrids()[i].
     * @return The pointer to the first host-resident Grid3D instance.
     */
    inline Grid3D<T> *hostGrids() { return _hGrids; }
    /**
     * This returns a pointer to the first device-resident Grid3D instance. A
     * pointer to (device-resident) instance i (for i < length) can be accessed
     * via deviceGrids()[i].
...
template <typename T>
MirroredGrid3DVector<T>::MirroredGrid3DVector(const uint length,
                                              const Grid3D<T> *grids)
    : _length(length)
{
