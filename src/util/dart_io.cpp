...
void readFrameXML(const int parent,
                  HostOnlyModel &model,
                  TiXmlElement *element,
                  std::string &dir,
                  const int version)
{
    for (TiXmlElement *child = element->FirstChildElement(); child != 0;
         child = child->NextSiblingElement())
    {
        if (strcmp(child->Value(), "frame") == 0)
        {
            ...
            {
                std::cerr << jointTypeStr
                          << " is not a recognized joint type; the options are "
                             "'rotational' and 'prismatic.'"
                          << std::endl;
            }
        ...
        {
            // TODO
            //            std::cerr << "unexpected node: " << child->Value() <<
            //            std::endl;
        }
    }
}
...
bool readModelXML(const char *filename, HostOnlyModel &model)
{
    ...
    {
        std::cerr << "expected 'model' at root node; got '" << root->Value()
                  << "'" << std::endl;
        return false;
    }
    ...
    // read size params
    for (TiXmlElement *child = root->FirstChildElement(); child != 0;
         child = child->NextSiblingElement())
    {
...
void writeGeomXML(const HostOnlyModel &model,
                  const int geom,
                  TiXmlElement *parent)
{
    ...
    {
        geomEl->SetAttribute("meshFile",
                             model.getGeometryMeshFilename(geom).c_str());
    }
    parent->LinkEndChild(geomEl);
}

void writeFrameXML(const HostOnlyModel &model,
                   const int frame,
                   TiXmlElement *parent)
{
    int joint = frame - 1;
...
void writeModelXML(const HostOnlyModel &model, const char *filename)
{
    ...
    for (std::map<std::string, float>::const_iterator it = sizeParams.begin();
         it != sizeParams.end();
         ++it)
    {
...
void saveState(const float *pose,
               ...
               std::string filename)
{
    ...
    for (int i = 0; i < dimensions; i++) pose_data[i] = pose[i];
...
LinearPoseReduction *loadLinearPoseReduction(std::string filename)
{
    ...
    LinearPoseReduction *reduction = new LinearPoseReduction(
        fullDimensions, reducedDimensions, A, b, mins, maxs, names.data());
...
ParamMapPoseReduction *loadParamMapPoseReduction(std::string filename)
{
    ...
    ParamMapPoseReduction *reduction = new ParamMapPoseReduction(
        fullDimensions, reducedDimensions, mapping, mins, maxs, names.data());
...
// LinearPoseReduction * loadLinearPoseReduction(std::string filename, const int
// fullDimensions, const int reducedDimensions) {
...
//    LinearPoseReduction* reduction; //= new
//    LinearPoseReduction(A,b,fullDimensions,reducedDimensions);
